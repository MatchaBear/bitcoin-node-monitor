...
- `bash -n ~/bitcoin-node-monitor/btc-monitor`
...
- Expect a new file: `~/bitcoin-node-monitor/peer_country_cache.txt` that grows as peers are geolocated
...
- Show history: `git --no-pager -C ~/bitcoin-node-monitor log --oneline -n 5 -- btc-monitor`
- Restore the previous version (one commit back):
  - `git -C ~/bitcoin-node-monitor restore --source=HEAD~1 -- btc-monitor`
- Re-run: `btcm`

B) Revert the last commit entirely (if it was only this change)
- `git -C ~/bitcoin-node-monitor revert --no-edit HEAD`
- `git -C ~/bitcoin-node-monitor push`

C) Restore from the backup tarball we created
- Backups are stored under: `~/backups/bitcoin/`
...
- Extract over the repo (this overwrites files to the saved state):
  - `tar -xzf ~/backups/bitcoin/bitcoin-YYYYmmdd-HHMMSS.tar.gz -C ~/bitcoin-node-monitor`
...
- If external geolocation APIs misbehave, the cache helps. If you still see issues, you can temporarily bypass the country lookup:
  - Open `~/bitcoin-node-monitor/btc-monitor`
...
- Output of:
  ...
  - `bash -n ~/bitcoin-node-monitor/btc-monitor && echo OK || echo FAIL`
